...
x = zeros(N,1);
S = zeros(T,N+K); S(1,:) = [x' u];

%% 2 - Entraînement de base
disp(['Apprentissage : n = ',num2str(1)]);
for t = 1:trainEnd
    x1 = (1 - delta*C*a)*x;    x2 = W_in*u + W*x + W_fb*Cible(t);
    x = x1 + delta*C*feval(f_RC,x2);
    S(t+1,:) = [x' u];    
end
...
for i = 2:imax
    ...
    S(1,:) = [x' u(:,1)];
    for t = 1:trainEnd
        x1 = (1 - delta*C*a)*x;    x2 = W_in*u + W*x + W_fb*NewCible(t,i-1);
        x = x1 + delta*C*feval(f_RC,x2);  
        S(t+1,:) = [x' u]; 
    ...
    for t = 1:trainEnd
        x1 = (1 - delta*C*a)*x;    x2 = W_in*u + W*x + W_fb*NewCible(t,i);
        x = x1 + delta*C*feval(f_RC,x2);  
        S(t+1,:) = [x' u];
    end
...
y_hat = [y_train zeros(1,T-trainEnd)];

clearvars y_train;

for t = trainEnd+1:T  
    y_hat(t) = W_out*S(t,:)';
    x1 = (1 - delta*C*a)*x;    x2 = W_in*u + W*x + W_fb*y_hat(t);
    x = x1 + delta*C*feval(f_RC,x2);
    S(t+1,:) = [x' u];
end
